...
from bale_inviter.adapters.http import HttpBaleAdapter, PHASE2_ALLOWED_METHODS
...
def test_phase2_whitelist_excludes_writes() -> None:
    assert "sendMessage" not in PHASE2_ALLOWED_METHODS
    assert "inviteUser" not in PHASE2_ALLOWED_METHODS
    assert PHASE2_ALLOWED_METHODS == frozenset({"getMe", "getChat", "getUpdates"})
...
def test_http_refuses_unallowed_method() -> None:
    ...
    with pytest.raises(RuntimeError, match="refuses method"):
        asyncio.run(adapter._call("sendMessage", {"chat_id": 1, "text": "hi"}))
